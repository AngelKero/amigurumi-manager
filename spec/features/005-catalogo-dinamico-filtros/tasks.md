...
- [ ] Crear `scripts/seed-catalogo-pruebas.php` **CLI-only** (guard `php_sapi_name() !== 'cli'`).
- [ ] Definir los ejes: 5 categorías oficiales × 3 materiales × 3 estados de stock
      (`en_stock`, `agotado`, `bajo_encargo`) × 3 bandas de precio (≤ $250, 250–600, ≥ $600) ×
      2 artesanos (`admin` #1, `artesana_ana` #2).
- [ ] Listar en consola la **matriz completa de combinaciones posibles** (todas las variantes).
- [ ] Insertar cada combinación vía `CreacionService::createCreation($data, null, admin)`,
...
- [ ] **Idempotencia:** consultar por nombre único antes de insertar; omitir si ya existe.
- [ ] Resumen de salida: creadas / omitidas / total de creaciones activas en BD.
- [ ] Flujo de despliegue documentado: `php setup.php` → `php scripts/seed-catalogo-pruebas.php`.

## 3. Contrato backend `estado_stock` (extensión quirúrgica)

- [ ] Añadir `estado_stock ∈ {en_stock, agotado, bajo_encargo}` a `CreacionService::getCatalog`
      y a `CreacionRepository::listCatalog`/`countCatalog` (prepared statements, R-08).
- [ ] Regresión Fase 3 tras el cambio: `php tests/test-subfase-3.6.5.php` (141) y
      `php tests/cuenta-aserciones.php` (**1,287**) en verde.

## 4. Catálogo reactivo (server-driven)

- [ ] `views/pages/catalogo_content.php`: eliminar mock `$catalogItems` + `foreach`; vaciar
...
- [ ] `src/js/modules/catalog.js`: `buildQuery()` (busqueda, categoria, `estado_stock`,
...
- [ ] `src/js/main.js`: asegurar `initCatalog()` en `DOMContentLoaded`.
- [ ] Verificación estable de seguridad DOM (H-004): cero datos interpolados en `innerHTML`.

## 5. Gate de testing 3-tier (obligatorio)

- [ ] Suite `tests/test-subfase-4.2.php`: contrato del endpoint con/sin filtros (datos +
...
- [ ] Sembrar dataset: `php setup.php` + `php scripts/seed-catalogo-pruebas.php`.
- [ ] Ejecutar suite: `php tests/test-subfase-4.2.php > logs/subfase-4.2-cli.log 2>&1` → 100%.
- [ ] Pruebas HTTP contra `php -S localhost:8000` → `logs/subfase-4.2-http.log` (filtros,
      centavos, paginación, `estado_stock`); divergencias según
      `docs/testing/protocolo-divergencia-cli-http.md` (H-015).
- [ ] Reporte ejecutivo `docs/testing/subfase-4.2-catalogo.md` con "Fallos Detectados &
      Correcciones Quirúrgicas".
- [ ] Regresión acumulada de fase: `php tests/test-fase-4-acumulado.php > logs/fase-4-acumulado.log 2>&1` (H-020).
- [ ] **HALT:** registrar en `009/tasks.md` y esperar aprobación explícita del usuario antes de 4.3.

## 6. Verificación & Cierre

- [ ] Criterios de aceptación de `spec.md` al 100% (`- [x]`).
- [ ] Actualizar `spec/constitution/roadmap.md`: subfase 4.2 → avance/Hecho según cierre.
- [ ] **HALT:** aprobación explícita del usuario antes de la siguiente feature.