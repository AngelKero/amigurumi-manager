<?php
/**
 * REST Controller: Restore Soft-Deleted Creation to Catalog (ADR-015)
 * Endpoint: POST /api/creaciones/restaurar.php
...
if ($id <= 0) {
    Response::error('El identificador "id" de la creación es obligatorio para su restauración.', 422);
}

// 5. Delegación a la capa de servicio con salvaguarda IDOR
try {
    $creacionService = new CreacionService();
    $result = $creacionService->restoreCreation($id, $currentUser);

    Response::success($result, 'Creación restaurada exitosamente en el catálogo.', 200);
