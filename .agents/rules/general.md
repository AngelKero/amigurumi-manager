...
- **Phase 2 - Layout & UI:** Build `index.html` (Catalog & Inventory view), `formulario.html` (Create/Edit form with file upload), `detalle.html` (Detail view with public checkout modal), and `pedidos.html` (Orders dashboard) using semantic HTML5 and Bootstrap 5 via CDN. Ensure navigation works across views. **All frontend views must strictly comply with the "Algodón Nórdico" Design System rules (`.agents/rules/ui-ux-design-system.md`).**
