...
1. **[CR-1] Out-of-Stock Guard (`cantidad_stock === 0`):**
   ...
   - Never allow a customer to check out an in-stock order if stock is zero. Offer a _"Solicitar Encargo Especial"_ trigger instead.
...
21. **On-Demand Exclusivity Badge (`es_sobre_encargo === 1`):**
    - Catalog items flagged as `es_sobre_encargo = 1` must display the distinctive textile tag _"Bajo Encargo Exclusivo"_, disabling immediate stock checkout and redirecting the user to custom commission scheduling.
