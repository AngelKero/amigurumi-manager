...
| **Orders Cards Redesign** | Rediseño de Pedidos: Cards 3x/2x & Cabecera Algodón Nórdico | **Completed & Verified** | Sustituida la tabla rígida de `pedidos.php` por una cuadrícula responsiva de cards artesanales pespunteadas (`.card-admin-pedido`), cabecera luminosa con sello oficial `isologo-sello-taller.svg` (>7:1 contraste), filtros textiles con tabs y contadores en vivo, miniaturas SVG de amigurumis, enlaces directos a WhatsApp, badges de pago tri-estado y módulo CSS dedicado (`orders.css`). |
...
- Rediseño de Pedidos con Cards Responsivas 3x/2x (`pedidos.php`, `pedidos_content.php`, `orders.css`, `orders.js`): sustitución completa de la tabla de datos y de la vista móvil duplicada por un grid responsivo unificado de tarjetas artesanales pespunteadas (`.card-admin-pedido.card-stitched`). Cabecera `.artisan-module-header` con sello oficial y contraste >7:1, filtros textiles interactivos con contadores reactivos, miniaturas SVG acolchadas, botones de contacto directo por WhatsApp (`wa.me`), badges tri-estado de cobro, notas de confección, inspección técnica modal y salvaguarda de restitución de inventario al cancelar.
