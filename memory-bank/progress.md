...
- Ergonomía del Formulario y Simulador de Márgenes (`formulario_content.php`, `margin-calculator.js`, `forms.css`): adaptada cuadrícula responsiva (`col-12 col-xl-7` y `col-12 col-xl-5`) evitando reducciones en pantallas XXL. Reestructurado el simulador con micro-cards dedicadas para `Margen de Utilidad` y `Retorno por Hora`, blindaje `text-nowrap` en cifras monetarias para erradicar fracturas de texto en múltiples líneas, formato conciso `$0.00/hr` y feedback pills a ancho completo uniforme.
