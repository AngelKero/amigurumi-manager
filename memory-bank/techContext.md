...
│   ├── eliminar.php            (Delete amigurumi with foreign key safeguard & image unlink)
...
- **Secure Image Uploads & File Cleanup:** Binary files validated by MIME type, size limit ($\le 5\text{MB}$), unique file naming, stored in `/uploads/`. On record deletion (`eliminar.php`) or image replacement (`actualizar.php`), associated local image files in `/uploads/` are unlinked via PHP `unlink()` to eliminate orphaned files.
