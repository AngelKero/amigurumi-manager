...
**Proyecto:** Amigurumi Manager (Micro-ERP & Catálogo Textil)  
...
- **Variantes Desarrolladas:**
  1. `isotipo-ovillo-corazon.svg`: Un ovillo de estambre texturizado con una hebra continua que dibuja un corazón y un gancho de crochet en diagonal.
  2. `isotipo-osito-amigurumi.svg`: Rostro geométrico y tierno de osito tejido con pespunte perimetral y orejitas tejidas.
  3. `isotipo-hebra-nórdica.svg`: Nube algodonada entrelazada con una lazada de estambre en espiral dorada.

### 2. Logotipo (Wordmark / Tipografía estilizada sin símbolo)
- **Definición:** La representación verbal exclusiva del nombre "Amigurumi Manager" mediante una construcción tipográfica personalizada basada en las curvas y terminales esféricas (*ball terminals*) de la fuente `Fraunces`, complementada por un pespunte inferior.
- **Variantes Desarrolladas:**
  1. `logotipo-amigurumi-manager.svg`: Wordmark principal en Ciruela Artesanal (`#8E5B74`) con hebra subyacente y punto de costura en la letra "i".
  2. `logotipo-taller-artesanal.svg`: Wordmark institucional con tipografía condensada y remate de pespunte suave.
...
- **Variantes Desarrolladas:**
  1. `imagotipo-horizontal.svg`: Isotipo de ovillo/gancho a la izquierda + Logotipo "Amigurumi Manager" con bajada "Micro-ERP & Catálogo Textil" a la derecha. Ideal para el `navbar` principal y encabezados de página.
  2. `imagotipo-vertical.svg`: Isotipo centrado en la parte superior + Logotipo y submarca centrados debajo. Ideal para tarjetas de presentación, pantalla de login, modales y portadas de reportes.
...
- **Variantes Desarrolladas:**
  1. `isologo-sello-taller.svg`: Sello circular con pespunte exterior bordado, texto arqueado sobre trayectoria SVG ("• AMIGURUMI MANAGER • TALLER DE CONFECCIÓN •"), anillo interior dorado y silueta de creación amigurumi en el centro.
  2. `isologo-medallon-garantia.svg`: Medallón festoneado con listón de garantía artesanal "100% HECHO A MANO", ideal para etiquetas de producto físico y certificados de autenticidad.
